...
| **Paper Runtime Stimulus CLI** | `services/validation/paper_runtime_stimulus_runner.py` | **AKTIV** (PR #2989, PR #2992) | Runtime-adjacent operator CLI for ARVP paper-window production (#2968/#2969). Default `--dry-run-preview`; `--publish` only after safety preflight (`MOCK_TRADING=true`, `DRY_RUN=true`, `MEXC_TESTNET=true`, rejects `LIVE_TRADING_CONFIRMED=yes`). Publishes canonical BTCUSDT 1m fixture to Redis `market_data`; optional `--runtime-relative` / `--base-ts-ms` shifts timestamps to current wall-clock (1m-aligned) while preserving breakout shape. No DB writes; LR remains NO-GO. Exit codes: 0 success / 1 usage / 2 safety or publish failure. |
