"""Local-dev helpers for run-scoped memory DB read/stale proof (#2603 / #2606).

Used by operator CLI, Makefile target, and local-only pytest fixtures.
Run-scoped seed via context_importer (local-dev, 127.0.0.1:8010) with cleanup.
"""
...
ENV_REAL_SURREALDB_MEMORY_SMOKE = "CDB_RUN_REAL_SURREALDB_MEMORY_SMOKE"
QUERY_CONFIG_REL = Path("infrastructure/config/surrealdb/context_query.local.yaml")
IMPORT_CONFIG_REL = Path(
...
def resolve_secrets_path() -> Path | None:
    for candidate in candidate_secrets_paths():
        try:
            if candidate.is_dir():
                return candidate
        except OSError:
...
def seed_memory_proof_bundle(
    ...
    *,
    ...
    import_config = _REPO_ROOT / IMPORT_CONFIG_REL
    ...
    if exit_code != 0:
        raise RuntimeError(f"context_importer apply failed with exit code {exit_code}")
