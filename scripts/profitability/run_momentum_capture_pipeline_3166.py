"""Controlled-lab pipeline pass for momentum_capture_v1 (Issue #3166).
...
Standalone profitability script that:
  ...
  - Computes ATR and candle body size for directional momentum detection
  - Runs long-only momentum capture signal logic (HIGH_VOL_CHAOTIC regime only)
  - Simulates fills via ExecutionSimulator
  - Produces per-window reports, aggregate economics, and evidence packet

Usage:
    python scripts/profitability/run_momentum_capture_pipeline_3166.py
    python scripts/profitability/run_momentum_capture_pipeline_3166.py --dry-run

Governance: Issue #3166
"""
...
STRATEGY_ID = "momentum_capture_v1"
...
CONTRACT_REF = "docs/evidence/profitability_candidate_momentum_capture_v1_3166.json"
...
OUTPUT_ROOT = Path(
    "artifacts/replay_reports/momentum_capture_v1_mexc_multi_window_3166"
)
...
def check_candle_integrity(candles: list[dict[str, Any]], label: str) -> None:
    for idx, c in enumerate(candles):
        for field in ("ts_ms", "high", "low", "close", "open"):
...
def evaluate_momentum_capture_candle(
    close: float,
    open_price: float,
    atr: float | None,
    regime_id: int,
    ...
    if not position_open:
        if current_market_regime != REGIME_HVC:
            return SignalDecision(
        ...
        if candle_body < atr * DIRECTIONAL_CANDLE_ATR_MULTIPLE:
            return SignalDecision(
                entry=False,
                exit_signal=False,
                stop_hit=False,
        ...
        if close <= open_price:
            return SignalDecision(
                ...
                exit_signal=False,
                stop_hit=False,
                reason="bearish_candle",
            )
        return SignalDecision(
            entry=True,
            exit_signal=False,
            stop_hit=False,
            reason="directional_momentum_entry",
            entry_price=close,
        )
    ...
    exit_price_candidate: float | None = None
    stop_hit = False
    exit_reason: str | None = None

    if atr is not None and entry_atr is not None and entry_atr > 0:
        if atr < entry_atr * EXIT_ATR_CONTRACTION_MULTIPLE:
            exit_reason = "atr_contraction"
            exit_price_candidate = close

    if atr is not None and entry_price is not None:
        stop_price = entry_price - atr * EXIT_TRAILING_STOP_ATR_MULTIPLE
        if close <= stop_price:
            exit_reason = "trailing_stop"
            exit_price_candidate = stop_price
            stop_hit = True
...
def write_per_window_report(
    ...
    metrics = {
        ...
        "hvc_window_pct": (
            window_result.regime_distribution.get("HIGH_VOL_CHAOTIC", {}).get("pct", 0)
        ),
    }
...
def build_economics_summary(
    ...
    for wr in results:
        win_econ = {
            ...
            "replay": {
                ...
                "hvc_window_pct": (
                    wr.regime_distribution.get("HIGH_VOL_CHAOTIC", {}).get("pct", 0)
                ),
            },
        }
    ...
    return {
        ...
        "summary_id": "econ-momentum-capture-v1-btcusdt-mexc-multi-window-3166",
        ...
        scenario_results.append(
            {
                ...
                "net_return": None,
                "max_drawdown": None,
                "notes": "See economics summary for per-window results.",
            }
        )

    return {
        "schema_version": EVIDENCE_SCHEMA_VERSION,
        "evidence_packet_id": "pep-momentum-capture-v1-btcusdt-mexc-3166",
        "candidate_id": CANDIDATE_ID,
        "strategy_id": STRATEGY_ID,
        "generated_at": _utc_now_iso(),
        "dataset_id": DATASET_ID,
        ...
        "regime_scorecard": {
            ...
            "summary": "Per-window regime labels were estimated via offline ADX/ATR heuristic using a distribution-based ATR p75 calibration rule (61.82 USD). controlled_lab_evidence only.",
        },
        ...
        "limitations": [
            ...
            "Long-only first pass only; short-side deferred.",
            ...
            "ATR p75 (61.82 USD) calibration used; HVC window share ~27%.",
        ],
        "safety_boundaries": [
            "LR remains NO-GO.",
            ...
            "controlled_lab_evidence only.",
            "No primary_breakout_v1 or range_mean_reversion_v1 tuning occurred.",
        ],
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="momentum_capture_v1 pipeline pass for #3166"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    print(f"momentum_capture_v1 pipeline (#3166) starting...")
    ...
    for idx, win in enumerate(selected):
        ...
        try:
            ...
            run_id = f"mc-{_sha256_text(_canonical_json({wid: wr.signals_total}))[:8]}-{idx + 1:04d}"
