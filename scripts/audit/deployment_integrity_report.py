"""
Deployment-domain DB integrity report for deployment approvals evidence.

The report validates row-level HMAC integrity for the deployment approvals
mirror storage and fails closed when the external integrity key is unavailable.
"""
...
TABLE_NAME = "deployment_approvals_mirror"
DISPLAY_NAME = "deployment_approvals"
...
REASON_SCHEMA_GAP = "DEPLOYMENT_SCHEMA_GAP"
REASON_SCHEMA_NOT_PROVABLE = "DEPLOYMENT_SCHEMA_NOT_PROVABLE_FROM_FIXTURE"
...
def load_rows_from_input_dir(input_dir: str) -> list[dict[str, Any]]:
    """Load deployment-domain fixtures from JSON files in ``input_dir``."""
...
def load_rows_from_postgres(
    ...
    """Load deployment-domain rows from PostgreSQL using the shared client factory."""
...
def load_schema_columns_from_postgres() -> dict[str, set[str]]:
    """Read actual column names for deployment-domain tables from PostgreSQL."""
    import psycopg2.extras
...
def build_schema_check(
    ...
    *,
    ...
    """Describe whether required deployment-domain fields are provably present."""
...
def build_report(
    ...
    *,
    ...
    """Build a deterministic report for deployment approval integrity."""
    resolved_key = resolve_integrity_key(key, env_var=DEPLOYMENT_INTEGRITY_KEY_ENV)
    schema_check = build_schema_check(rows, schema_columns=schema_columns)
    entries = [
    ...
    return {
        "schema": "governance.deployment_integrity_report.v1",
        "status": (
            "PASS"
        ...
        "integrity_key_env": DEPLOYMENT_INTEGRITY_KEY_ENV,
        "table": {
...
def build_verification_md(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            "**Naming note:** `deployment_approvals` is the governance-domain "
            "name; repo-visible storage uses `deployment_approvals_mirror`. "
            "`yaml_evidence_path` should point at the external "
            "`DELIVERY_APPROVED.yaml` evidence."
        ),
    ]
    ...
    lines.extend(
        [
            "",
            "---",
            "*Generated by `deployment_integrity_report.py`*",
            "",
        ]
    )
    return "\n".join(lines)
...
def generate_report(
    ...
    if from_db:
        schema_columns = load_schema_columns_from_postgres()
        rows, _ = load_rows_from_postgres(schema_columns=schema_columns)
    elif input_dir:
        rows = load_rows_from_input_dir(input_dir)
        schema_columns = None
    else:
        raise ValueError("either input_dir or from_db must be provided")

    report = build_report(rows, key=key, schema_columns=schema_columns)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Validate deployment approvals mirror integrity and emit an audit "
            f"report. Fails closed when {DEPLOYMENT_INTEGRITY_KEY_ENV} is missing."
        )
    )
    ...
    source_group.add_argument(
        "--input-dir",
        help=(
            "Directory containing fixture files such as "
            "deployment_approvals_mirror.json or deployment_approvals.json"
        ),
    )
    source_group.add_argument(
        "--from-db",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if report["status"] == "PASS":
        print(
            f"PASS: validated {report['total_entries']} deployment-domain rows.",
            file=sys.stderr,
        )
        return 0

    print(
        "FAIL: deployment-domain integrity verification reported gaps or invalid rows.",
        file=sys.stderr,
    )
    return 1
